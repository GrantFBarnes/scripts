...
fn run_menu(start_idx: usize, distribution: &mut Distribution) -> Result<(), io::Error> {
    ...
    match options[selection] {
        ...
        "Install Packages" => run_menu_install_packages(0, distribution)?,
        "Exit" => return Ok(()),
        _ => (),
    }
...
fn run_menu_install_packages(
    ...
    match options_display[selection] {
        "Exit" => return Ok(()),
        _ => run_menu_category_select(options_value[selection], 0, false, distribution)?,
    }

    run_menu_install_packages(selection + 1, distribution)
}

fn run_menu_category_select(
    category: &Category,
    start_idx: usize,
    ...
    if missing_desktop_environment && selection == 0 {
        ...
        // not exit
        run_menu_package_select(&options_value[selection], distribution)?;
        run_menu_category_select(
            category,
            selection + 1,
            show_all_desktop_environments,
...
fn run_menu_package_select(
    ...
    match method {
        ...
        InstallMethod::Flatpak => run_menu_flatpak_remote_select(package, distribution)?,
...
fn run_menu_flatpak_remote_select(
    package: &Package,
    distribution: &mut Distribution,
) -> Result<(), io::Error> {
